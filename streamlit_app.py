...
st.set_page_config(page_title="🔥 Гал эсэргүүцэх чадварын тооцоо", page_icon="🔥", layout="wide")
...
LANG_TEXT = {
    "en": {
        ...
        "section3": "3. Reinforcement",
        "lbl_b": "Width (mm)",
        "lbl_h": "Height (mm)",
        "lbl_H0": "Column height (mm)",
        "lbl_kL": "Length factor",
        "lbl_c1": "c₁ – Bottom (mm)",
        "lbl_c2": "c₂ – Top (mm)",
        "lbl_c3": "c₃ – Middle (mm)",
        "lbl_Rbn": "Concrete (MPa)",
        "lbl_Rsn": "Steel (MPa)",
        "lbl_rho": "Density (kg/m³)",
        "lbl_W": "Moisture (%)",
        "lbl_tb": "Concrete T (°C)",
        "lbl_t0": "Initial T (°C)",
...
def gamma_steel(t):
    """Steel strength reduction factor vs temperature."""
...
def phi_by_lambda(lam):
    """Stability factor vs slenderness."""
...
def compute_step(tau, b, h, c1, c2, c3, Rbn, Rsn, As1, As2, As3, t0, Np, phi, aRed, kbS):
    """Calculate capacity and temps at time tau."""
    ...
    return {"tau": tau, "ts1": ts1, "ts2": ts2, "ts3": ts3, "g1": g1, "g2": g2, "g3": g3, "delta": delta, "Nu": Nu, "ok": Nu >= Np}
...
def calculate(inputs):
    ...
    return {
        ...
        "l0": l0, "lambda": lambda_val, "phi": phi, "N0": N0, "N0_pass": N0 >= Np,
        "Np": Np, "chart_rows": chart_rows
    }
...
with st.sidebar:
    ...
    lang = st.radio("", ["en", "ru", "mn"], format_func=lambda x: {"en": "🇬🇧 English", "ru": "🇷🇺 Русский", "mn": "🇲🇳 Монгол"}[x])
...
with col1:
    ...
    tmax = st.number_input("Max time (min)", value=180, min_value=60, step=30)
    phi_manual = st.number_input("φ (manual)", value=0.0, step=0.001, format="%.3f")
    
    if st.button(t("btn_calculate"), use_container_width=True, type="primary"):
        inputs = {
            "b": b, "h": h, "H0": H0, "kL": kL, "c1": c1, "c2": c2, "c3": c3,
            "Rbn": Rbn, "Rsn": Rsn, "rho": rho, "W": W, "tb": tb, "t0": t0,
            "As1": As1, "As2": As2, "As3": As3, "Np": Np, "tmax": int(tmax),
        ...
        with col2:
            ...
            with col_m1:
                st.metric("Capacity", f"{result['N0']:.0f} kN")
            with col_m2:
                st.metric("Load", f"{result['Np']:.0f} kN")
            with col_m3:
                st.metric("Status", "✅ PASS" if result["N0_pass"] else "❌ FAIL")
            with col_m4:
            ...
            # Cross-section
            st.subheader(t("cross_section"))
            if len(result["chart_rows"]) > 0:
                cr = result["chart_rows"][0]
                col_sec1, col_sec2 = st.columns([2, 1])
                
                with col_sec1:
                    svg_html = draw_cross_section_svg(
                        b, h, c1, c2, c3, As1, As2, As3,
                ...
                with col_sec2:
                    ...
                    **Color legend:**
                    ...
                    if len(result["chart_rows"]) > 1:
                        ...
                        selected_tau = st.slider("Time (min)", 0, max_tau, 0)
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=chart_times, y=chart_Nus, name="Capacity",
                mode="lines", line=dict(color="#ff7a1a", width=3), fill="tozeroy"))
            fig.add_trace(go.Scatter(x=chart_times, y=[result["Np"]]*len(chart_times), 
                name="Load", mode="lines", line=dict(color="#5aa9ff", width=2, dash="dash")))
            fig.update_layout(height=400, hovermode="x unified")
            st.plotly_chart(fig, use_container_width=True)

st.markdown("---")
st.caption(f"© {datetime.now().year} Fire Resistance Calculator | Made with ❤️")
